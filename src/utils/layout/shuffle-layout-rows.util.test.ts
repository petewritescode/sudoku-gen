...
describe('shuffleLayoutRows', () => {
  test('randomly orders the layout rows', () => {
    ...
    seedRandom('lorem');
    expect(shuffleLayoutRows(BASE_LAYOUT)).toEqual(expected1);

    seedRandom('ipsum');
    expect(shuffleLayoutRows(BASE_LAYOUT)).toEqual(expected2);

    seedRandom('dolor');
    expect(shuffleLayoutRows(BASE_LAYOUT)).toEqual(expected3);
  });
});
